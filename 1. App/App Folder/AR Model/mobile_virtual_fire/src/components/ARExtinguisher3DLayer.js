...
/**
 * High-Performance Three.js WebGL 3D AR Extinguisher:
 * - Direct Hold-to-Spray & Release-to-Stop Interaction (Touch & Hold on 3D Model)
 * - True 3D GLB model rendering anchored directly at bottom-right in the AR camera scene
 * - Realistic operational action animations: forward aim tilt, trigger compression, high-frequency pressure recoil
 * - High-speed cryogenic nozzle fog stream emission during hold
 * - Clean UI with no unnecessary buttons
 */
const buildExtinguisher3DHtml = (isExtinguishing, glbBase64) => `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
...
<body>
  ...
  <script>
    ...
    // Convert Base64 data to binary ArrayBuffer safely
    ...
    // 4. Load & Normalize User's 3D GLB Model (fire_exting.glb)
    function loadGLBModel() {
      ...
      loader.parse(
        ...
        function (gltf) {
          ...
          // Compute exact bounding box and center pivot at center of extinguisher
          const box = new THREE.Box3().setFromObject(loadedModel);
          const size = box.getSize(new THREE.Vector3());
          const center = box.getCenter(new THREE.Vector3());
          const maxDim = Math.max(size.x, size.y, size.z) || 1.0;

          // Normalize scale to fit the viewport perfectly
          const normalizedScale = 2.15 / maxDim;
          loadedModel.scale.set(normalizedScale, normalizedScale, normalizedScale);

          // Center the 3D model exactly at group origin
          ...
          // Enhance materials with glossy metallic finish
    ...
    // Press & Hold Start (Finger down on 3D model)
    ...
    // Release Hold (Finger lifted from 3D model)
    ...
    function animate() {
      ...
      // Operational Animation State
      if (isDischarging) {
        // Dynamic aim tilt towards scene center + high-speed recoil vibration
        ...
        // Idle state: Smooth return to center and gentle 360° inspection rotation
...
function ARExtinguisher3DLayer({
  ...
  const handleMessage = (event) => {
    try {
      ...
      if (data.type === 'EXTINGUISHER_HOLD_STATE' || data.type === 'EXTINGUISHER_TOGGLE') {
    ...
    <View style={styles.extinguisherWrapper} pointerEvents="box-none">
      ...
      >
        ...
        <Text style={styles.promptText}>
          {isExtinguishing ? 'DISCHARGING CO₂' : 'HOLD 3D MODEL TO SPRAY'}
        </Text>
      </View>
